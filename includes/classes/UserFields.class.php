...
class UserFields
{
    ...
    public function displayOutput()
    {
        ...
        $this->renderHeader();
        if (!iMEMBER) {
            $this->html .= "<div class='alert alert-info' style='margin-bottom: 0'>Dieser Bereich ist nur fÃ¼r unsere Mitglieder sichtbar.</div>";
        } else {
            $this->renderFieldsOwn();
        }
        $this->renderFooter();
        echo $this->html;
    }

    private function renderFieldsOwn()
    {
        ...
        // get cats
        $result = dbquery(
            "SELECT field_cat_id, field_cat_name FROM ".DB_USER_FIELD_CATS." ORDER BY field_cat_order"
        );

        if (dbrows($result)) {
            while ($data = dbarray($result)) {
                $cats[] = array($data['field_cat_id'], $data['field_cat_name']);
            }
        }
        ...
        foreach ($cats as $k => $cat) {
            if ($cat[0] == 0) {
                $bodyTabHtml .= "<div role='tabpanel' class='tab-pane".($k == 0 ? " active" : "")."' id='cat-".$cat[0]."'>\n";
                $bodyTabHtml .= "<table class='table tbl-border table-responsive table-striped profile-table'>\n";
                if (iADMIN || $this->userData['user_hide_email'] == 0) {
                    $bodyTabHtml .= $this->basicOutputField($locale['u064'], hide_email($this->userData['user_email']), "profile_user_email");
                }
                $bodyTabHtml .= $this->basicOutputField($locale['u066'], showdate("longdate", $this->userData['user_joined']), "profile_user_joined");
                if ($this->userData['user_groups']) {
                    ...
                    $bodyTabHtml .= "<strong>".$locale['u057']."</strong>\n";
                    $bodyTabHtml .= "</td>\n</tr>\n";
                    for ($i = 0; $i < count($user_groups); $i++) {
                        $bodyTabHtml .= "<tr>\n<td class='tbl1'>\n";
                        $bodyTabHtml .= "<a href='".FUSION_SELF."?group_id=".$user_groups[$i]."'>".getgroupname($user_groups[$i])."</a>\n";
                        $bodyTabHtml .= "</td>\n<td class='tbl1' align='right'>\n";
                        $bodyTabHtml .= getgroupname($user_groups[$i], true)."\n";
                        $bodyTabHtml .= "</td>\n</tr>\n";
                    }
                }
                $bodyTabHtml .= "</table>\n";
            ...
            $result = dbquery(
                "SELECT * FROM ".DB_USER_FIELDS."
				WHERE field_cat=".$cat[0]."
        ...
        foreach ($cats as $k => $cat) {
            $this->html .= "<li role='presentation'".($k == 0 ? " class='active'" : "").">\n";
            $this->html .= "<a role='tab' data-toggle='tab' aria-controls='cat-".$cat[0]."' href='#cat-".$cat[0]."'>".$cat[1]."</a>\n";
            $this->html .= "</li>\n";
        }
        $this->html .= "</ul>\n";
        $this->html .= "<div class='tab-content profile-tab-content'>\n";
        $this->html .= $bodyTabHtml;
        $this->html .= "</div>\n";
    }
    ...
    private function renderHeader()
    {
        ...
        $this->html .= "<div class='panel-heading clearfix'>\n";

        if (iMEMBER && $userdata['user_id'] != $this->userData['user_id']) {
            $this->html .= "<div class='pull-right'>\n";
            $this->html .= "<a class='button alt-button' href='".BASEDIR."messages.php?msg_send=".$this->userData['user_id']."' title='".$locale['u043']."'><i class='fa fa-envelope'></i></a>\n";
            if (iADMIN && checkrights("M") && $this->userData['user_level'] != "103" && $this->userData['user_id'] != "1") {
                $this->html .= "<a class='button alt-button' href='".ADMIN."members.php".$aidlink."&amp;step=log&amp;user_id=".$this->userData['user_id']."' title='".$locale['u054']."'><i class='fa fa-book'></i></a>\n";
            }
            $this->html .= "</div>\n";
        }

        $this->html .= "<div class='pull-left'><img src='".IMAGES."avatars/".$this->userData['user_avatar']."' class='user-profile-avatar img-responsive' alt='".$locale['u062']."' title='".$locale['u062']."' style='margin-right: 10px;'/></div>\n";
        $this->html .= "<h2 class='panel-title' style='margin-bottom: 5px;'>".$this->userData['user_name']."</h2>\n";
        $this->html .= "<p><span class='label label-".$this->getLabelColor($this->userData['user_level'])."'>".getuserlevel($this->userData['user_level'])."</span>\n</p>\n";
        $this->html .= "<span class='small text-muted'>".$locale['u067'].": ".$lastVisit."</span>";
        $this->html .= "</div>\n";
        $this->html .= "<div class='panel-body'".(iMEMBER ? "  style='min-height: 300px;'" : "").">\n";
    }
    ...
    private function renderAdminOptionsOwn()
    {
        ...
        if (count($groups_cache) > 0 && is_array($groups_cache)) {
            ...
            if (iADMIN && checkrights("UG") && $user_groups_opts) {
                ...
                $this->html .= $locale['u061'].": <select name='user_group' class='textbox' style='width:100px;margin-bottom:0;'>\n".$user_groups_opts."</select>\n";
