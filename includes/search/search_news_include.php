...
if ($_GET['stype'] == "news" || $_GET['stype'] == "all") {
    ...
    $rows = 0;
    if ($fieldsvar) {
        $rows = dbcount("(news_id)",
            DB_NEWS,
            groupaccess('news_visibility')
            ." AND ".$fieldsvar." AND (news_start='0'||news_start<=".time().")"
            ." AND (news_end='0'||news_end>=".time().") "
            .($_GET['datelimit'] != 0
                ? " AND news_datestamp>=".(time() - $_GET['datelimit'])
                : ""
            ));
    }
    ...
    if ($rows != 0) {
        $items_count .= THEME_BULLET."&nbsp;"
            ."<a href='".FUSION_SELF."?stype=news&amp;stext=".$_GET['stext']."&amp;".$composevars."'>"
            .$rows." ".($rows == 1 ? $locale['n401'] : $locale['n402'])." ".$locale['522']
            ."</a><br />\n";
        $result = dbquery(
        ...
        while ($data = dbarray($result)) {
            ...
            $search_result .= "<div class='media'>\n<div class='media-body'>\n";
            $search_result .= "<h4 class='media-heading'>"
                ."<a href='".BASEDIR."news.php?readmore=".$data['news_id']."'>".$data['news_subject']."</a>"
                ."</h4>\n";
            $search_result .= "<div class='media-text' style='width:auto; height:auto; overflow:auto'>\n<p>\n"
                .$text_frag
                ."</p>\n<span class='text-muted'>"
                .$locale['global_070'].profile_link($data['user_id'], $data['user_name'], $data['user_status'])
                ." ".$locale['global_071'].showdate("longdate", $data['news_datestamp'])
                ."</span><br />\n<span class='small text-muted'>"
                .$text_c." ".($text_c == 1 ? $locale['520'] : $locale['521'])." ".$locale['n403']." ".$locale['n405'].", "
                .$text_c2." ".($text_c2 == 1 ? $locale['520'] : $locale['521'])." ".$locale['n403']." ".$locale['n406']
                ."</span>\n</div>\n";
            $search_result .= "</div>\n</div>\n<hr />\n";
