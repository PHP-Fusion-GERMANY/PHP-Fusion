...
if ($_GET['stype'] == "custompages" || $_GET['stype'] == "all") {
	...
	if ($rows != 0) {
		$items_count .= THEME_BULLET."&nbsp;"
            ."<a href='".FUSION_SELF."?stype=custompages&amp;stext=".$_GET['stext']."&amp;".$composevars."'>"
            .$rows." ".($rows == 1 ? $locale['c401'] : $locale['c402'])." ".$locale['522']
            ."</a><br />\n";
		$result = dbquery(
			"SELECT * FROM ".DB_CUSTOM_PAGES."
			WHERE ".groupaccess('page_access')." AND ".$fieldsvar."
			ORDER BY ".$sortby." ".($_GET['order'] == 1 ? "ASC" : "DESC")
            .($_GET['stype'] != "all" ? " LIMIT ".$_GET['rowstart'].",10" : "")
		);	 
		while ($data = dbarray($result)) {
            ...
            $search_result .= "<h4 class='media-heading'>"
                ."<a href='".BASEDIR."viewpage.php?page_id=".$data['page_id']."'>".$data['page_title']."</a>"
                ."</h4>\n";
            $search_result .= "<div class='media-text' style='width:auto; height:auto; overflow:auto'>\n<p>\n"
                .$text_frag
                ."</p>\n<span class='small text-muted'>"
                .$subj_c." ".($subj_c == 1 ? $locale['520'] : $locale['521'])." ".$locale['c403']." ".$locale['c404'].", "
                .$text_c." ".($text_c == 1 ? $locale['520'] : $locale['521'])." ".$locale['c403']." ".$locale['c405']
                ."</span>\n</div>\n";
            $search_result .= "</div>\n</div>\n<hr />\n";
