    ...
    if (!$settings['hide_userprofiles'] || iMEMBER) {
        ...
        if ($rows != 0) {
            $items_count .= THEME_BULLET."&nbsp;"
                ."<a href='".FUSION_SELF."?stype=members&amp;stext=".$_GET['stext']."&amp;".$composevars."'>"
                .$rows." ".($rows == 1 ? $locale['m401'] : $locale['m402'])." ".$locale['522']
                ."</a><br />\n";
            $result = dbquery("
            ...
            while ($data = dbarray($result)) {
                $search_result .= "<li>".profile_link($data['user_id'], $data['user_name'], $data['user_status'])."</li>\n";
            }
            $search_result .= "</ul>\n";
